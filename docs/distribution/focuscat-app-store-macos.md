...
| **Fresh clone or new machine** – no `apps/focuscat/.local/`                   | [§ Fresh clone](#2-fresh-clone-no-local)                        |
...
```bash
codesign --force --options runtime --entitlements apps/focuscat/src-tauri/Entitlements.plist --sign "Apple Distribution: Your Name (TEAM_ID)" target/universal-apple-darwin/release/bundle/macos/Focuscat.app
mkdir -p apps/focuscat/.local/output
xcrun productbuild --sign "3rd Party Mac Developer Installer: Your Name (TEAM_ID)" --component target/universal-apple-darwin/release/bundle/macos/Focuscat.app /Applications apps/focuscat/.local/output/Focuscat.pkg
```

**3. Upload** (from `apps/focuscat`):
...
You have the repo but no `apps/focuscat/.local/`. You need the same secrets and assets as on a machine that already ships Focuscat.
...
1. Get the files above from a secure store or from another machine that has them. Create `apps/focuscat/.local/` and put the profile, `.p8`, and `.env` there.
...
3. **AuthKey for altool:** From `apps/focuscat`:  
...
- **altool** looks for the API key only in fixed paths (`~/private_keys`, etc.). If the key lives in `apps/focuscat/.local/`, create a symlink: `ln -sf "$(pwd)/.local/AuthKey_<KEY_ID>.p8" ~/private_keys/`.
